...
class Finance_taskbase(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'val':
            ...
            input_ids = []
            labels = []
            input_ids = self.tokenizer.encode(self.questions[idx])
...
class Finance_task(ABDataset):    
    def create_dataset(self, root_dir: str, split: str, transform, 
                       classes: List[str], ignore_classes: List[str], idx_map: Optional[Dict[int, int]]):
        return Finance_taskbase(root_dir, split, transform, classes, ignore_classes, idx_map)

# a = Finance_taskbase('/data/zql/datasets/finance_task', 'train', None, None, None, None)
# a.__getitem__(0)