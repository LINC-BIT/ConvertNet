...
class FeatureAlignmentLoraAlg(BaseAlg):
    def get_required_models_schema(self) -> Schema:
        return Schema({
            'main': FeatureAlignmentLoraModel
        })
    ...
    def run(self, scenario: Scenario, hyps: Dict) -> Dict[str, Any]:
        ...
        assert isinstance(self.models['main'], FeatureAlignmentLoraModel) # for auto completion
        ...
        # add LoRA
        from ..lora_ft.lib import add_lora_ab_to_model, absorb_lora, train_only_lora
        qkv_layers = []
        for l in hyps['qkv_layers_name']:
            qkv_layers += l
        add_lora_ab_to_model(self.models['main'].model, qkv_layers, len(hyps['qkv_layers_name'][0]) == 1, 
                             hyps['lora_r'])
        logger.debug(f"model after adding lora: {self.models['main'].model}")
        
        device = self.models['main'].device
        trained_params = train_only_lora(self.models['main'].model)
        ...
        self.models['main'].model = absorb_lora(self.models['main'].model)
        ...
        plt.plot([i['iter'] for i in accs], [i['acc'] for i in accs])
        plt.xlabel('iteration')
        plt.ylabel('acc')
