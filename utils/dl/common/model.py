...
def get_model_size(model: torch.nn.Module, return_MB=False, exclude_embedding=False):
...
def get_model_num_params(model: torch.nn.Module, return_M=True):
...
def get_model_latency(model: torch.nn.Module, model_input_size: Tuple[int], sample_num: int, 
    ...
    if isinstance(model_input_size, tuple):
        ...
        dummy_input = model_input_size.to(device)
...
def get_parameter(model: torch.nn.Module, param_name: str):
    if param_name is None:
        return None
    m = get_module(model, '.'.join(param_name.split('.')[0: -1]))
    if m is None:
        return None
    return getattr(
        m,
...
class LayerActivationWithCustomFunc:
    """Collect the input and output of a middle module of a PyTorch model during inference.
    ...
    Example:
        ...
        >>> layer_activation.remove()
    """
    def __init__(self, layer: torch.nn.Module, input_hook=lambda x: x, output_hook=lambda x: x):
        """Register forward hook on corresponding layer.
    ...
    def _hook_fn(self, module, input, output):
        ...
        output = self.output_hook(output)
